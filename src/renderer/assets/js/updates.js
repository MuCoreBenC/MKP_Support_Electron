...
function parseManifestToUI(manifest, currentAppVersion) {
  const cleanCurrent = String(currentAppVersion || '0.0.0').replace(/^v/i, '');
  const uiVersions = [];
...
async function loadLocalManifest() {
  try {
    ...
    const localManifest = window.mkpAPI && typeof window.mkpAPI.readLocalManifest === 'function'
      ? await window.mkpAPI.readLocalManifest()
      : null;
  ...
  return `
    <div class="bg-white dark:bg-[#252526] rounded-xl border border-gray-200 dark:border-[#333] p-5 mb-4 shadow-sm transition-all hover:shadow-md hover:border-gray-300 dark:hover:border-[#444] cursor-pointer group collapse-item ${defaultExpandedClass}" onclick="toggleCollapse(this)">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-3">
          <button class="px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${btnClass}" ${isDisabled ? 'disabled' : ''} ${btnAction}>
            ${btnText}
          </button>
...
async function manualCheckAppUpdate(btnElement) {
  ...
  try {
    ...
    const currentAppVersion = window.mkpAPI ? await window.mkpAPI.getAppVersion() : '0.0.0';
    const cleanCurrent = currentAppVersion.replace(/^v/i, '');
    globalVersions = parseManifestToUI(remoteManifest, currentAppVersion);
    renderVersions();

    if (resetEngine) resetEngine();

    if (compareVersionsFront(remoteManifest.latestVersion, cleanCurrent) > 0) {
      ...
      const updateResult = await window.mkpAPI.applyHotUpdate({
        ...
        expectedVersion: remoteManifest.latestVersion
      });
...
function initUpdateModeSetting() {
  ...
  const savedMode = localStorage.getItem('update_mode') || 'auto';
...
async function handleRollback(btnElement, targetVersion) {
  ...
  try {
    const manifest = await fetchAppManifestWithFallback();
    const pureVersion = targetVersion.replace(/^v/i, '');
    const targetData = (manifest.history || []).find((item) => item.version === pureVersion);
